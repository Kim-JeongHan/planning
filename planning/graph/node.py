...
class Node:
    ...
    def __init__(
        self,
        state: Union[Tuple[float, ...], np.ndarray, List[float]],
    ...
    def distance_to_state(self, state: Union[Tuple[float, ...], np.ndarray, List[float]]) -> float:
        ...
        Returns:
            The Euclidean distance
        """
        return np.linalg.norm(self.state - np.array(state))
    ...
    def change_parent(self, new_parent: 'Node', new_cost: float):
    ...
    def __repr__(self) -> str:
        ...
        parent_str = 'None' if self.parent is None else f'({", ".join([f"{x:.2f}" for x in self.parent.state])})'
