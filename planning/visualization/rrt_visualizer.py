...
class RRTVisualizer:
    ...
    def visualize_branches(
        ...
        print(f"Visualizing {total_branches} exploration branches ({len(success_leaves)} success, {len(failure_leaves)} failures)...")
        ...
        for branch_idx, leaf_node in enumerate(failure_leaves):
            ...
            for i in range(len(path) - 1):
                ...
                # Extract positions
                parent_pos = parent_state[:3] if len(parent_state) >= 3 else np.append(parent_state, 0)
            ...
            end_pos = path[-1].state[:3] if len(path[-1].state) >= 3 else np.append(path[-1].state, 0)
            self.server.scene.add_icosphere(
