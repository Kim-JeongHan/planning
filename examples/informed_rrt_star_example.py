"""Informed RRT* algorithm example with mixed obstacle types."""
...
def main(seed: int = 42, save_image: bool = False) -> None:
    """Informed RRT* with mixed obstacle types and 3D visualization."""
    print("=== Informed RRT* with Mixed Obstacles (Boxes & Spheres) ===\n")
    ...
    print("Planning with Informed RRT*...")
    print(f"  Start: {start_state}")
    print(f"  Goal: {goal_state}")
    print(f"  Bounds: {map_env.get_bounds()}")
    print(f"  Max iterations: {informed_rrt_star.max_iterations}")
    print(f"  Step size: {informed_rrt_star.step_size}")
    print(f"  Goal tolerance: {informed_rrt_star.goal_tolerance}")
    print(f"  Radius gain: {informed_rrt_star.radius_gain}")
    print("  Sampler: InformedSampler (ellipsoidal heuristic)\n")
    ...
    if path is not None:
        ...
        print(f"Path length: {informed_rrt_star.get_path_length():.2f}")
        print(f"Total nodes in graph: {len(informed_rrt_star.graph.nodes)}")
        print(f"Total edges in graph: {len(informed_rrt_star.graph.edges)}\n")
        ...
        print("   Blue lines: Optimal path found by Informed RRT*")
        print("   Red lines: Other edges in the graph")
        print("   Orange boxes/spheres: Obstacles")
        print("\nNote: Informed RRT* uses ellipsoidal sampling to focus search")
        print("      on the region that can improve the current best solution.")

    else:
        print("\n No path found!")
        print("Try increasing max_iterations or changing the seed.")
        # Visualize the graph even if no path is found
        visualizer.visualize_graph(informed_rrt_star)

    # Statistics
    stats = informed_rrt_star.get_stats()
    print("\nStatistics:")
    ...
    if save_image:
        ...
        def handle_save(client: viser.ClientHandle) -> None:
            ...
            save_docs_image(client, "informed_rrt_star_example.png")
            print(" Image saved to docs/images/informed_rrt_star_example.png")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Informed RRT* algorithm example")
